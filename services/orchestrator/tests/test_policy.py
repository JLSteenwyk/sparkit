...
def test_policy_stage_cost_positive() -> None:
    assert estimate_stage_cost("retrieval", units=2) > 0


def test_policy_should_stop_by_cost() -> None:
    state = BudgetState(elapsed_s=10.0, spent_usd=1.9)
    assert should_stop_early(state, max_latency_s=120, max_cost_usd=2.0, reserve_next_stage_usd=0.2) is True
...
def test_generation_cost_kimi_k25_uses_cache_hit_miss_output_rates() -> None:
